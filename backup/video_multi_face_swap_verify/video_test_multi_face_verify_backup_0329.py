...
def video_test_multi_face(args):
    ...
    for i in tqdm(range(int(all_f))):
        ...
        bboxes, target_aligned_images = target_face_align(landmarkModel,i, frame, args.verify_img_path, video_name, args.image_size)
...
# 타겟 이미지 수평 맞춤
def target_face_align(landmarkModel,frame, target_image, verify_img_path, video_name, image_size=224):
    aligned_imgs =[]
    ...
    bboxes, landmarks = landmarkModel.gets(target_image, verify_img)
