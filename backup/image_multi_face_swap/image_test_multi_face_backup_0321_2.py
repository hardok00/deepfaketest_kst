...
def face_align(landmarkModel, image_path, image_size=224):
    ...
    for path in img_list:
        img = cv2.imread(path)
        landmark = landmarkModel.s_gets(img)
...
def s_faces_align(landmarkModel, image_path, image_size=224):
    aligned_imgs =[]
    if os.path.isfile(image_path):
        img_list = [image_path]
    else:
    ...
    for path in img_list:
        ...
        source_path = f"asset/{name}_source"
        os.makedirs(source_path, exist_ok=True)
        
        bboxes, landmarks = landmarkModel.s_gets(img)
        
        # print(landmarks)
        
        for source_count, landmark in enumerate(landmarks):
            if landmark is not None:
                aligned_img, back_matrix = align_img(img, landmark, image_size)
                aligned_imgs.append([aligned_img, back_matrix])
                                            
                # 타겟 이미지 여러개 추출
                cv2.imwrite(source_path + f'/{name}_source_{source_count}.png', aligned_img)
                np.save(source_path + f'/{name}_source_back_{source_count}.npy', back_matrix)

        source_list = [os.path.join(source_path, x) for x in os.listdir(source_path) if x.endswith('png') or x.endswith('jpg') or x.endswith('jpeg')]
        
        print(source_list)
    return aligned_imgs


def t_faces_align(landmarkModel, image_path, image_size=224):
    aligned_imgs =[]
    target_count = 0
    if os.path.isfile(image_path):
        img_list = [image_path]
    ...
    for path in img_list:
        ...
        bboxes, landmarks = landmarkModel.t_gets(img)
...
if __name__ == '__main__':
    ...
    if args.need_align:
        ...
        source_aligned_images = s_faces_align(landmarkModel, args.source_img_path)
        target_aligned_images = t_faces_align(landmarkModel, args.target_img_path, args.image_size)
