...
def video_test_multi_face(args):
    ...
    bboxes, landmark = landmarkModel.gets(id_img)
    if landmark is None:
    ...
    for i in tqdm(range(int(all_f))):
        ...
        frame_folder = f"asset/{video_name}_target/{video_name}_target_{i}"
        
        # 타겟 이미지 얼굴 디텍트
        # target_aligned_images = target_face_align(landmarkModel, frame, args.image_size)
        bboxes, target_aligned_images = target_face_align(landmarkModel,i, frame, video_name, args.image_size)
        if i == 0:
            main_image = frame_folder + target_aligned_images[1][2]
        ...
        for idx, target_aligned_image in enumerate(target_aligned_images):
            ...
            if i > 0:
                try :
                    result = DeepFace.verify(img1_path = main_image, img2_path = frame_folder + target_aligned_images[idx % len(target_aligned_images)][2])
                    print(result)
                except:
                    continue
...
# 타겟 이미지 수평 맞춤
def target_face_align(landmarkModel,frame, image, video_name, image_size=224):
    aligned_imgs =[]
