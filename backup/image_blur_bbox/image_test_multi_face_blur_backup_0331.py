...
from utils.facial_landmarks import FaceLandmarks
...
def image_test_multi_face(args, result):
    ...
    cv2.imwrite(os.path.join(args.output_dir, os.path.basename(target_name.format(4))), result)
...
def faces_align(landmarkModel, image_path, image_size=224):
    ...
    for path in img_list:
        ...
        print(convexhull.dtype)
        
        mask = np.zeros((h,w),np.uint8)
        cv2.polylines(mask, np.int32(convexhull), True, 255, 3)
        cv2.fillConvexPoly(mask, np.int32(convexhull), 255)
        frame_copy = cv2.blur(frame_copy, (27, 27))
        frame_copy = cv2.GaussianBlur(img, (0, 0), 20)
...
if __name__ == '__main__':
    ...
    if args.need_align:
        ...
        result = faces_align(landmarkModel, args.target_img_path, args.image_size)
    os.makedirs(args.output_dir, exist_ok=True)
    image_test_multi_face(args, result)
